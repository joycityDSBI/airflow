...
def etl_statics_daily_kpi(**context):
    ...
    for td_str in target_date:
        ...
        try:
            ...
            # 3. 성공 시 출력
            print(f"✅ 쿼리 실행 성공! (Job ID: {query_job.job_id})")
            print(f"■ {td_str} statics_daily_kpi_market Batch 완료")
...
def etl_statics_weekly_kpi(**context):
    ...
    for td_str in target_date:
        ...
        # 일요일
        print(f"📝 statics_weekly_kpi_market 대상주간: {start_of_week} ~ {end_of_week}")
    ...
    print("✅ statics_weekly_kpi_market ETL 완료")
...
def etl_statics_monthly_kpi(**context):
    ...
    for td_str in target_date:
        ...
        # 일요일
        print(f"📝 statics_monthly_kpi_country 대상월: {start_str} ~ {end_str}")
               

        # ETL 작업 수행
        query = f"""
        MERGE INTO datahub-478802.datahub.statics_monthly_kpi_country as target
        USING (
                SELECT 
                TA.datekey,
                TA.joyple_game_code,
        ...
        try:
            ...
            # 3. 성공 시 출력
            print(f"✅ 쿼리 실행 성공! (Job ID: {query_job.job_id})")
            print(f"■ {td_str} statics_monthly_kpi_country Batch 완료")
    ...
    print("✅ statics_monthly_kpi_country ETL 완료")
    ...
    for td_str in target_date:
        ...
        # 일요일
        print(f"📝 statics_monthly_kpi_market 대상월: {start_str} ~ {end_str}")
               

        # ETL 작업 수행
        query = f"""
        MERGE INTO datahub-478802.datahub.statics_monthly_kpi_market as target
        USING (
                SELECT 
                TA.datekey,
                TA.joyple_game_code,
        ...
        try:
            ...
            # 3. 성공 시 출력
            print(f"✅ 쿼리 실행 성공! (Job ID: {query_job.job_id})")
            print(f"■ {td_str} statics_monthly_kpi_market Batch 완료")
    ...
    print("✅ statics_monthly_kpi_market ETL 완료")
