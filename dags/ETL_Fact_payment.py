...
with DAG(
    ...
    def etl_pre_payment_deduct_user(target_date: list):
        ...
        for td in target_date:
            ...
            WHERE RegistredTime >= TIMESTAMP('{start_utc.strftime("%Y-%m-%d %H:%M:%S %Z")}')
            AND RegistredTime < TIMESTAMP('{end_utc.strftime("%Y-%m-%d %H:%M:%S %Z")}')
    ...
    def etl_pre_payment_deduct_order(target_date: list):
        ...
        for td in target_date:
            ...
            WHEN NOT MATCHED BY source AND (target.update_timestamp >= TIMESTAMP('{start_utc.strftime("%Y-%m-%d %H:%M:%S %Z")}')
              AND target.update_timestamp < TIMESTAMP('{end_utc.strftime("%Y-%m-%d %H:%M:%S %Z")}')) THEN
            DELETE  
            ;
            """
    ...
    def etl_f_common_payment_total(target_date: list):
        ...
        for td in target_date:
            ...
            query = f"""
                ...
                WHERE a.timestamp >= '2025-11-26'
                    AND a.timestamp < '2025-11-27'
