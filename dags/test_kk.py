...
def aggregate_data(**context):
    ...
    source_df['conversation_date'] = pd.to_datetime(source_df['event_time_kst']).dt.date

# 2. conversation_id 와 conversation_date를 기준으로 그룹화 (groupby)
    aggregated_df = source_df.groupby(['conversation_id', 'conversation_date']).agg(
        user_email=('user_email', 'first'),
        space_name=('space_name', 'first'),
        conversation_flow=('content', list)
    ).reset_index()

    # 3. 데이터 타입 변환 (conversation_date는 1번 과정에서 이미 date 타입이 되었습니다)
    aggregated_df['conversation_id'] = aggregated_df['conversation_id'].astype(str)
    
    # [선택사항] JSON 직렬화(XCom Push) 시 date 객체 오류를 방지하려면 문자열로 변환해 두는 것이 안전할 수 있습니다.
    # aggregated_df['conversation_date'] = aggregated_df['conversation_date'].astype(str)
...
def sync_with_notion(**context):
    ...
    # 설정 값 가져오기
    NOTION_DB_ID = get_var("NOTION_DB_ID_INJOY_MONITORINGDATA", "30dea67a5681803c844ceea9a0976f92")
