...
def extract_audit_logs(**context):
        ...
        USING (
            WITH raw_log AS (
                ...
                WHERE service_name = 'aibiGenie'
                    ...
                    AND event_time >= CAST(DATE(NOW()) - INTERVAL 120 DAYS AS TIMESTAMP) - INTERVAL 9 HOURS
    ...
    try:
            ...
            WHERE DATE(event_time_kst) >= DATE(NOW()) - INTERVAL 120 DAYS
    ...
    query_history_merge = f"""
        ...
        USING (
            ...
            WHERE query_source.genie_space_id IS NOT NULL
                ...
                AND end_time >= CAST(DATE(NOW()) - INTERVAL 120 DAYS AS TIMESTAMP) - INTERVAL 9 HOURS
...
def merge_final_monitoring_data(**context):
    ...
    USING (
        ...
        WHERE DATE(A.event_time_kst) >= DATE(NOW()) - INTERVAL 120 DAYS
