...
BQ_TABLE_ID = 'sensorTower_active_users'
...
def sensortower_download_revenue_api(start_date, end_date, APP_ID, SENSORTOWER_TOKEN):
    
    client = init_clients()["bq_client"]
    
    # 다운로드와 매출액 API 호출
    active_users_url = f"https://api.sensortower.com/v1/unified/usage/active_users?app_ids={APP_ID}&time_period=day&start_date={start_date}&end_date={end_date}&data_model=DM_2025_Q2&auth_token={SENSORTOWER_TOKEN}"
    response = requests.get(active_users_url, timeout = 120)
    data = response.json()
    ...
    items = data if isinstance(data, list) else data.get("estimates", [])
    ...
    numeric_cols = ['android_users', 'ipad_users', 'iphone_users']
...
def fetch_data_in_weekly_batches(total_start_str: str, total_end_str: str, APP_ID: str, SENSORTOWER_TOKEN: str):
    ...
    while current_start <= end_date:
        ...
        try:
            sensortower_download_revenue_api(batch_start_str, batch_end_str, APP_ID, SENSORTOWER_TOKEN)
...
def app_id_active_users_fetch_load(APP_ID_LIST: list, SENSORTOWER_TOKEN: str):
...
with DAG(
    dag_id='SENSORTOWER_active_users',
    default_args=default_args,
    description='센서타워 API를 통해 Active users를 가져와 BigQuery에 적재',
    schedule= '40 21 * * *', # KST 06:40 AM 매일 실행 -> UTC 21:40 PM 전날 실행
    start_date=datetime(2025, 1, 1),
    catchup=False,
    tags=['SensorTower', 'active_users', 'bigquery'],
) as dag:

    seonsortower_active_users_fetch_load_task = PythonOperator(
        task_id='app_id_active_users_fetch_load',
        python_callable=app_id_active_users_fetch_load,
        op_kwargs={
